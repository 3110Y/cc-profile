...
func (p *ProfileRepository) Get(ctx context.Context, id string) (*entity.Profile, error) {
	pr := entity.Profile{}
	err := p.db.GetContext(ctx, &pr, "SELECT * FROM profile WHERE id = $1", id)
	return &pr, err
}

func (p *ProfileRepository) List(ctx context.Context, onPage uint64, page uint64) (*[]entity.Profile, error) {
	profileList := []entity.Profile{}
	offset := (onPage * page) - onPage
	err := p.db.SelectContext(
		ctx,
		&profileList,
		"SELECT * FROM profile ORDER BY create_at ASC LIMIT $1 OFFSET $2 ",
		onPage,
		offset,
	)
